...
(function () {
    ...
    (function () {
      ...
      window.fetch = async function (...args) {
        const url = args[0];
        if (typeof url === "string" && url.includes("/activate") || url.includes("/validate")) {
          console.log("🔧 Intercepted key validation fetch:", url);
      ...
      WebSocket.prototype.send = function (data) {
        try {
          if (typeof data === "string" && data.includes("validate") && data.includes("hGUXX-hGUXX-hGUXX")) {
            console.log("🧩 Intercepted WebSocket key validation attempt.");
            // block or modify here if needed
          }
        } catch (e) {}
    ...
    (function () {
      ...
      class _0x5f8785 {
        ...
        async ["init"]() {
            ...
            // 🔐 Force our hardcoded key into login flow
            const key = "hGUXX-hGUXX-hGUXX";
            const input = this.find("#activation-key");
            if (input) input.value = key;
        
            // ⏩ Skip storage check, go straight to login
            await this.login();
        }
        ...
        async ["login"]() {
          const _0x47c4ca = "hGUXX-hGUXX-hGUXX";
    ...
    _0x5f8785.prototype.init = async function () {
      this.createUI();
      this.setupDrag();
      this.initializePredictor();
      setInterval(() => this.trackBalance(), 1000);
    };
    const waitForPredictor = setInterval(() => {
      if (typeof _0x5f8785 !== 'undefined') {
        clearInterval(waitForPredictor);
        const core = new _0x5f8785();
        core.init();
      }
    }, 1000);
  })(); // End of UserScript
